...
- `pp.Canvas(preset, *, width=None, abc=None, strict_vectors=False)` — multi-panel paper figure builder. Stage rows via `canvas.add_row(pp.PanelAxes(...), pp.PanelGrid(...), pp.PanelText(...), pp.PanelImage(...))`. Save via `canvas.savefig('fig.pdf' | 'fig.svg' | 'fig.png')`.
